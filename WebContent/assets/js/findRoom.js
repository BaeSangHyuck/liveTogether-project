...
$("select[name='max']").focusout(function(){
	if(Number($(this).val()) < Number($("select[name='min']").val())){
		$(this).val(Number($("select[name='min']").val())).prop("selected", true);
	}
})

// 월세 최솟값이 최댓값보다 커지면 최댓값을 최솟값에 맞춰주기
$("select[name='min']").focusout(function(){
	if(Number($(this).val()) > Number($("select[name='max']").val())){
		$("select[name='max']").val(Number($(this).val())).prop("selected", true);
	}
})	
