								...
								<c:forEach var="inquiry" items="${inquiryList}">
									<tr>
										...
										<td class="ttd ttd6"><div class="button2-wrapper">
												...
												<button type="button" class="button2"
													onclick="location.href='${pageContext.request.contextPath}/board/InquiryDeleteOk.bo?inquiryNumber=${inquiry.getInquiryNumber()}'">삭제</button>
											</div></td>
									</tr>
								</c:forEach>
							</c:when>
						...
						<c:choose>
							<c:when test="${search != null}">
								<td><c:if test="${startPage > 1}">
										<a
											href="${pageContext.request.contextPath}/board/InquiryAdminOk.bo?page=1">&lt;&lt;&nbsp;&nbsp;</a>
										<a
											href="${pageContext.request.contextPath}/board/InquiryAdminOk.bo?page=${startPage -1}">&lt;&nbsp;&nbsp;</a>
										...
										<c:choose>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/board/InquiryAdminOk.bo?page=${i}"><c:out
														value="${i}" /></a>&nbsp;&nbsp;
								</c:otherwise>
										</c:choose>
									</c:forEach> <c:if test="${endPage < realEndPage}">
										<a
											href="${pageContext.request.contextPath}/board/InquiryAdminOk.bo?page=${endPage + 1}">&nbsp;&nbsp;&gt;</a>
										<a
											href="${pageContext.request.contextPath}/board/InquiryAdminOk.bo?page=${realEndPage}">&nbsp;&nbsp;&gt;&gt;</a>
									</c:if></td>
							</c:when>
							<c:otherwise>
								<td><c:if test="${startPage > 1}">
